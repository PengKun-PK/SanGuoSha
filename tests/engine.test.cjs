...
test('judgement card is revealed before retrial skills are offered',async()=>{const a=engine(),g=a.game(['simayi','zhangfei']);const [p,t]=g.players;t.skills=[];const seen=[];g.askSkill=async(who,id,ctx,event)=>{seen.push({id,event,card:ctx.card.name,pass:ctx.check(ctx.card),logged:g.logs.some(l=>l.includes('亮出')&&l.includes('闪'))});return id==='guicai';};g.ask=async(who,req)=>req.kind==='select'?{cards:[who.hand[0]],target:null}:false;p.hand=[a.makeCard('杀','heart',5)];g.deck=[a.makeCard('闪','club',3)];const r=await g.judge(t,{reason:'乐不思蜀',check:c=>c.suit==='heart'});assert.deepEqual(seen,[{id:'guicai',event:'judgeCard',card:'闪',pass:false,logged:true}]);assert.equal(r.card.name,'杀');assert.ok(r.ok);});
