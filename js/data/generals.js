...
const SKILL_TEXT = {
  ...
  fanjian:['反间','出牌阶段限一次，你可以令一名其他角色从你的手牌中盲选一张并猜其花色，然后其获得并展示该牌。若猜错，该角色受到你造成的1点伤害。'],
...
/* 暂时停用的武将：不出现在选将界面、不分配给电脑、也不会被化身抽到。
   于吉的【蛊惑】实现有问题，先下架。 */
const DISABLED_GENERALS = new Set(['yuji']);
const pickableGenerals = () => Object.keys(GENERALS).filter(id=>!DISABLED_GENERALS.has(id));
