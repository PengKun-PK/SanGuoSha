...
Game.prototype.runPhase=async function(p,phase){
 ...
 await basePhase.call(this,p,phase);if(p.alive&&!this.over)await this.trigger('phaseAfter',ctx);await this.flushLoss();
};
const baseTurn=Game.prototype.playerTurn;
Game.prototype.playerTurn=async function(p){
 if(p.marks.turned){this.curPlayer=p;this.phase='';await EX.flip(this,p);return;}
 if(p.hasSkill('huashen')&&!p.marks.forms){await EX.getForms(this,p,2);await EX.transform(this,p);}
 await baseTurn.call(this,p);
 if(p.marks.extraTurn&&p.alive&&!this.over){const t=p.marks.extraTurn;delete p.marks.extraTurn;if(t.alive){this.log(`${t.name}获得额外回合。`,true);await this.playerTurn(t);}}
};
...
Game.prototype.damage=async function(ctx){
 ...
 if(ctx.applied&&!ctx.cancelled){await this.trigger('damageDone',ctx);if(wasLinked&&ctx.nature&&!ctx.chain){const targets=this.alivePlayers().filter(q=>q!==t&&q.marks.linked);t.marks.linked=false;for(const q of targets){q.marks.linked=false;await this.damage({...ctx,target:q,chain:true});}}}
};
